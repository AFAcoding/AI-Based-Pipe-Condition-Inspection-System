...
# Quantitative evaluation on the validation/test set (mAP, precision, recall)
metrics = modelv5n.val()

results = modelv5n('/content/YOLO_Imp2.mp4', save=True, conf=0.25, iou=0.5, verbose=True )

import matplotlib.pyplot as plt

gt_label_dir = "/content/labels/test"
class_names = modelv5n.names
...
for result in results:
    ...
    # Load and draw ground truth boxes (in green)
    img_filename = os.path.basename(img_path)
    label_path = os.path.join(gt_label_dir, img_filename.replace(".jpg", ".txt"))
    if os.path.exists(label_path):
        ...
        print(f"Missing label file for: {img_filename}")
    ...
    plt.imshow(img_rgb)
    plt.axis('off')
    plt.title(f"Predictions vs Ground Truth — {img_filename}")
    plt.show()
